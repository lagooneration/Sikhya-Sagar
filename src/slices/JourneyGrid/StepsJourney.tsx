...
export async function StepsJourney({ id }: Props) {
  const client = createClient();
  const product = await client.getByID<Content.StepsDocument>(id);


//   const price = isFilled.number(product.data.seats_left)
//     ? `$${(product.data.seats_left / 100).toFixed(2)}`
//     : "Price Not Available";
    ...
    <div className="group relative mx-auto w-full max-w-72 px-8 pt-4 ">
      ...
      <div className="flex items-center justify-between ~text-sm/2xl">
        <span className="inline-flex items-center gap-1">
          <FaUserGraduate className="text-brand-blue" /> <span className="text-sm">16 Seats Left</span>
        </span>
        <span>{price}</span>
      </div>
      ...
      <div className="absolute inset-0 mt-20 flex items-center justify-center opacity-0 transition-opacity duration-200 group-hover:opacity-100">
        <ButtonLink field={product.data.schedule_consultant}>Schedule a Demo</ButtonLink>
      </div>
    </div>
  );
}